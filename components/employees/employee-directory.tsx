...
export function EmployeeDirectory() {
  const { activeProject, role } = useAuth();
  ...
  const loadEmployees = useCallback(() => {
    if (!activeProject && role === 'PROJECT_MANAGER') return;
    const list = getEmployees(activeProject?.id);
    setEmployees(list);
  }, [activeProject, role]);
  ...
  return (
    <div className="space-y-6">
      {/* Top Header (Icon-Based on Mobile) */}
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="text-xs text-slate-500 mt-0.5 truncate">
            {activeProject?.name || 'All Construction Sites'}
          </p>
        </div>
      ...
      {/* Filters Bar (Icon-Based Status Selector) */}
      <div className="p-3.5 sm:p-4 rounded-3xl bg-white border border-slate-200 shadow-sm flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder="Search by worker name, Iqama ID, or title..."
            value={searchQuery}
      ...
      {filtered.length === 0 ? (
        <div className="p-12 text-center rounded-3xl bg-white border border-slate-200">
          ...
          <p className="text-xs text-slate-400 mt-1">
            Try adjusting your search criteria or onboard a new employee to this project.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((emp) => (
            <div
              key={emp.id}
              className="p-4 rounded-3xl bg-white border border-slate-200 shadow-sm hover:border-blue-400 transition-all group flex flex-col justify-between"
            >
              <div>
              ...
              <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-1">
                  ...
                  >
                    {emp.status === 'ACTIVE' ? <CheckCircle2 className="w-4 h-4 text-emerald-600" /> : <XCircle className="w-4 h-4 text-slate-400" />}
                  </button>
                  <button
                    type="button"
